...
type GenerateRedeemCodesRequest struct {
	...
	// concurrency 已从兑换码类型中移除：并发数是运维参数，不是可售卖的商品。
	// 调整并发请走 PUT /admin/users/:id（编辑用户）或批量限额接口。
...
type CreateAndRedeemCodeRequest struct {
	...
	Type          string     `json:"type" binding:"omitempty,oneof=balance subscription invitation"` // 不传时默认 balance（向后兼容）；concurrency 见 GenerateRedeemCodesRequest 说明
