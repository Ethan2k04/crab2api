  ...
  keys: {
    ...
    useKeyModal: {
      ...
      codex: {
        ...
        configTomlHintUnix:
          '步骤：用文本编辑器打开 ~/.codex/config.toml 这个文件。如果 .codex 文件夹不存在，先运行 mkdir -p ~/.codex 创建。打开后用下面的内容整个替换掉 config.toml 里原有的内容并保存，保存后需完全重启 Codex 才会生效。',
        configTomlHintWindows:
          '步骤：用文本编辑器打开 %userprofile%\\.codex\\config.toml 这个文件（按 Win+R，输入 %userprofile%\\.codex\\config.toml 即可打开）。如果 .codex 文件夹不存在，先手动新建一个 .codex 文件夹。打开后用下面的内容整个替换掉 config.toml 里原有的内容并保存，保存后需完全重启 Codex 才会生效。',
      ...
      grok: {
        ...
        codexConfigTomlHint:
          'Codex 官方：wire_api 仅支持 "responses"；优先 env_key，勿与 experimental_bearer_token 混用；非 OpenAI 网关默认 supports_websockets = false（Crab2API 仍可接客户端 WS 并桥接到 HTTP/SSE）。合并前备份 ~/.codex/config.toml。',
        note:
