...
// codex-claude's guidance now lives entirely in the config.toml comments
// (see generateCodexClaudeFiles), so this tab gets no separate note box.
const showPlatformNote = computed(() => activeClientTab.value !== 'codex-claude')
...
// Turns a (possibly multi-line) translated string into TOML comment lines.
const commentBlock = (text: string) => text.split('\n').map((line) => `# ${line}`).join('\n')
...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready Crab2API setups.
  ...
  const configContent = `# Grok Build CLI → Crab2API Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for Crab2API Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches Crab2API).
...
description = "Grok 4.5 via Crab2API (Responses)"
...
# Imagine tools: model IDs go to Crab2API media endpoints (not the text [model.*] catalog).
...
# Grok/Crab2API path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
